...
import com.app.springboot.util.article_images;
import com.app.springboot.util.data_util;
    ...
    public article_images upload_image(@RequestParam(value = "file") MultipartFile file, Model model) throws IOException {
        if(!file.isEmpty()) {
            ...
            if (file_name == null || !file_name.contains(".")) {
                return new article_images(400,"文件名无效",new data_util("/layuimini/image_article/","无"));
            }
            
            String fuffixname = file_name.substring(file_name.lastIndexOf(".")).toLowerCase();
            
            if (!ALLOWED_EXTENSIONS.contains(fuffixname)) {
                return new article_images(400,"文件类型不允许，仅支持jpg/jpeg/png/gif/bmp",new data_util("/layuimini/image_article/","无"));
            }
